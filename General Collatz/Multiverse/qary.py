...
#: bases giving a deterministic Miller-Rabin test for every n < 3.3 * 10^24
...
def is_prime(n: int) -> bool:
    ...
    Trial division below 10^6 -- fast enough there and obviously correct --
    and deterministic Miller-Rabin above it.  The bases in `_MR_BASES` are
    the first twelve primes, which are known to give a *deterministic* test
    for every n < 3.317 * 10^24; the density experiment of the paper's
    "beyond modularity" section reaches ~10^12, so the test never leaves that
    range.  This is a speed change only: the predicate is the same one.
    """
    if n < 2:
        return False
    for p in _MR_BASES:
    ...
    if n >= 3317044064679887385961981:
        raise ValueError("is_prime: deterministic range exceeded")
    d, s = n - 1, 0
    while d % 2 == 0:
        d //= 2
        s += 1
...
def primality_map() -> PredicateMap:
    ...
    The tempting heuristic -- "primes have density 0, so the growing branch is
    negligible and the map behaves like n -> n/2 (even), n+1 (odd), which is
    T_{1,1} and provably collapses" -- is NOT available, and the paper says so
    explicitly.  Density in N is not density along an orbit, and the orbits of
    this map are measurably biased towards primes -- by roughly a factor of two
    at every scale measured (see the density table of the paper's "beyond
    modularity" section, generated by generate_data.table_primedensity; no
    figure is repeated here, so this docstring cannot go stale).  The bias is
    far below the ln2/ln3 = 63.09% needed for divergence and is falling,
    which is why every orbit is
    *expected* to be periodic -- but that is Conjecture "primality map", not a
    theorem, because it is a statement about the invariant measure of the map
    rather than about the density of the primes.
    """
    return PredicateMap([
