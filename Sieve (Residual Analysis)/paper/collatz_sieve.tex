...
% the algorithm names appear inside italic proposition bodies, and Latin
% Modern has no small-caps italic
\newcommand{\alg}[1]{\textnormal{\textsc{#1}}}
...
\author{RobinCodes\thanks{The correctness audit, the Rocq development and this paper were
produced in collaboration with an artificial-intelligence system; see the note on the use of
artificial intelligence at the end of this paper.}\\[1mm]
...
searches have confirmed it for all $n$ below roughly $2^{71}$~\cite{barina}.
...
For odd $n$ and every $q \geq 0$, $\alg{Eval}(n,q) = \Tmap^{q}(n)$.
\end{proposition}
...
Table~\ref{tab:data} lists the program's output for $q \leq 24$; the run itself
is committed as \texttt{output.txt} beside the program, so the table can be
checked line by line against it. The survivor counts reproduce the classical
Terras sieve counts, and the residue sets agree elementwise with an
independent step-by-step implementation of $\Tmap$ that shares no code with
the bit-twiddling version.
...
The program \texttt{Residual\_Analysis.py} whose correctness is established here is the author's.
The correctness audit that produced this paper, the Rocq development
\texttt{proofs/CollatzSieve.v} described in Section~\ref{sec:rocq}, and the text of this paper were
produced in collaboration with an artificial-intelligence system (Claude, Anthropic), at the
author's direction.

No result here rests on machine assertion. The Rocq development is axiom-free and contains no
admitted proofs, so Theorem~\ref{thm:main} is checked by the Rocq kernel rather than asserted; the
prose arguments are stated so as to be checkable by hand against that development; the computed
data of Section~\ref{sec:data} were cross-checked against an independent implementation sharing no
code with the one under proof; and Section~\ref{sec:limits} states explicitly what is \emph{not}
proved.
